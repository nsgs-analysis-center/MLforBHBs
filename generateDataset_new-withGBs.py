...
def convertToYoloCoords(bboxIndices, tCoords, fCoords, yLimMin, yLimMax, asinhLinearWidth = 0.0005,classId = 0):
    """
    ...
    return f'{classId} {xCenter:.6f} {yCenter:.6f} {width:.6f} {height:.6f}'
...
def makeTrainingImage(
    ...
    numWDBsRange=[3, 4],
    wdFRange=[1e-4, 1e-2],
    wdFdotRange=[0.0, 1e-16],
    wdIotaRange=[0, np.pi],
    wdPsiRange=[0, 2 * np.pi],
    wdPhi0Range=[0, 2 * np.pi],
    wdM1Range=[0.8, 1.5],
    wdM2Range=[0.8, 1.5],
    wdLogDistanceRange=[-4, -3],
    wdSinbetaRange=[-1.0, 1.0],
    wdLambdaRange=[0.0, 2 * np.pi],
    wdVerticalPadding=3,
    wdClassId=1,
    gb=None,
    wdResponseWrapper=None,
    dt = 2.5,
    ...
    wdArgsDict = {}
    wdRangesList = [wdFRange, wdFdotRange, wdIotaRange, wdPsiRange, wdPhi0Range,
                    wdM1Range, wdM2Range, wdLogDistanceRange, wdSinbetaRange, wdLambdaRange]
    wdParamNames = ['f', 'fdot', 'iota', 'psi', 'phi0', 'm1', 'm2',
                    'logdistance', 'sinbeta', 'lam']
    for k, pname in enumerate(wdParamNames):
        wdArgsDict[pname] = rng.uniform(wdRangesList[k][0], wdRangesList[k][1], numWDBs)

    wdArgsDict['distance'] = 10**(wdArgsDict['logdistance'])
    wdArgsDict['beta'] = np.arcsin(wdArgsDict['sinbeta'])

    if numWDBs > 0 and gb is None:
        gb = WDWave(force_backend="fastlisaresponse_cpu")
    if numWDBs > 0 and wdResponseWrapper is None:
        wdResponseWrapper = instantiateWDResponseWrapper(
            TObsTot / ASTRONOMICAL_YEAR, dt, gb
        )
    ...
    for i in range(numWDBs):
        try:
            waveA, waveE, waveT = getWDResponse(
                f=wdArgsDict['f'][i], fdot=wdArgsDict['fdot'][i],
                iota=wdArgsDict['iota'][i], psi=wdArgsDict['psi'][i],
                phi0=wdArgsDict['phi0'][i], lam=wdArgsDict['lam'][i],
                beta=wdArgsDict['beta'][i], dt=dt, Tobs=TObsTot,
                m1=wdArgsDict['m1'][i], m2=wdArgsDict['m2'][i],
                distance=wdArgsDict['distance'][i],
                responseWrapper=wdResponseWrapper, gb=gb,
            )
            ...
            cleanTotTimeSeries += waveA_real
            wdIndivTimeSeries.append(waveA_real)
        except Exception as e:
            print(f"WDB {i} failed: {e}")
            traceback.print_exc()
            wdIndivTimeSeries.append(np.zeros(numTotTimes))

    randomArgsDict['wd_params'] = {
        k: (v.tolist() if isinstance(v, np.ndarray) else v)
        for k, v in wdArgsDict.items()
    }
    ...
    if filename is not None:
        ...
        wdBBoxes = []
        for i in range(numWDBs):
            wdWDM, wdFMinIdx, _ = getWDMMatrix(
                wdIndivTimeSeries[i], nT, nF, fmin=fmin, fmax=fmax, dt=dt
            )
            wdBBoxes.append(
                getWDBoundingBox(
                    wdWDM,
                    freq=wdArgsDict['f'][i],
                    absFMinIdx=wdFMinIdx,
                    waveletDuration=waveletDuration,
                    verticalPadding=wdVerticalPadding,
                    wdbIdx=i,
                )
            )
        fCoordSpacing_matrix = 1 / (2 * 2.5 * nF)
        fCoordSpacing_bbox   = 1 / waveletDuration / 2
        print("dt*nF =", 2.5*nF, " waveletDuration =", waveletDuration)
        print("matrix spacing =", fCoordSpacing_matrix, " bbox spacing =", fCoordSpacing_bbox)
        ...
        with (Path(imageFolder).parent.parent / "labels" / testTrainVal / (filename + ".txt")).open('w') as file:  # TODO: maybe make this a bit nicer with relpath or something rather than parent.parent and whatever
            txtFileStr = ""
            for bbox in BBoxes:      # BHB -> class 0
                fmaxConversionFunc = fmax if fmax != np.inf else fCoords[-1]
                yoloString = convertToYoloCoords(bbox, tCoords, fCoords, fmin,
                                                 fmaxConversionFunc, classId=0)
                txtFileStr += f'{yoloString}\n'

            for bbox in wdBBoxes:    # WD -> class 1
                fmaxConversionFunc = fmax if fmax != np.inf else fCoords[-1]
                yoloString = convertToYoloCoords(bbox, tCoords, fCoords, fmin,
                                                 fmaxConversionFunc, classId=wdClassId)
                txtFileStr += f'{yoloString}\n'
...
fileLock = None
workerGB = None
workerWDResponseFunc = None

def initWorker(dt, tObs, tObsTot, lock):
    global workerResponseFunc, fileLock, workerGB, workerWDResponseFunc
    workerResponseFunc = instantiateResponseFunc(dt=dt, Tobs=tObs, allModes=False)
    workerGB = WDWave(force_backend="fastlisaresponse_cpu")
    workerWDResponseFunc = instantiateWDResponseWrapper(
        tObsTot / ASTRONOMICAL_YEAR, dt, workerGB
    )
    fileLock = lock
...
def generateSingleTask(args):
    ...
    tObsTotConst = 2 * ASTRONOMICAL_YEAR
    ...
    while not success:
        try:
            makeTrainingImage(
                ...
                gb=workerGB,
                wdResponseWrapper=workerWDResponseFunc,
...
def generateDataset(totalImages, trainValTest, baseDir = "imageData", topThresh = 0.5, leftThresh = 0.1, vmin = 1e-6, waveformFile = None, useSavedWaveforms = False):
    ...
    with Pool(
        processes = optimalCores,
        initializer = initWorker,
        initargs = (dtConst, tObsConst, tObsTotConst, lock),   # <-- added tObsTotConst
        maxtasksperchild = tasksPerChildLimit,
    ) as pool:
