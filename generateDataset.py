...
def makeTrainingImage(
    ...
    # Step 3b: Place the numBHBs waveforms into a total padded array and into their own padded arrays -----------------------------------------------------
        ...
        #totTimes = np.arange(numTotTimes) * dt
        #plt.figure()
        #plt.plot(totTimes, cleanTotTimeSeries, rasterized = True)
        #plt.xlabel('Time (s)')
        #plt.ylabel('TDI A')
        #plt.title('Total Time Series')
        #plt.savefig('Poster Time Series Plot.png', dpi = 600)
